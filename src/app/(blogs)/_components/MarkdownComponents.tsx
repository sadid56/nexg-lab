...
const CodeBlock = ({ className, children }: any) => {
  ...
  return (
    <div className='relative group mb-6 rounded-2xl overflow-hidden border border-muted/50 bg-gradient-to-br from-muted/50 to-muted/30 backdrop-blur-sm shadow transition-all duration-300'>
      ...
      <pre className='p-6 pt-14 overflow-x-auto scrollbar-thin scrollbar-thumb-muted scrollbar-track-transparent'>
        <code className={`font-mono text-sm leading-relaxed ${className || ""}`}>{children}</code>
      </pre>
...
export const markdownComponents = {
  ...
  p: ({ node, children, ...props }: any) => {
    ...
    return (
      <p className='mb-6 text-base md:text-lg text-foreground/80 leading-relaxed' {...props}>
        {children}
      </p>
    );
  },
  ...
  li: ({ children, ...props }: any) => (
    <li
      className='text-foreground/80 leading-relaxed relative pl-2 before:absolute before:left-[-1rem] before:content-["→"] before:text-primary/60 before:font-bold'
      {...props}
    >
      {children}
    </li>
  ...
  hr: ({ ...props }: any) => <hr className='border-0 h-px bg-gradient-to-r from-transparent via-muted to-transparent my-12' {...props} />,
  ...
  img: ({ src, alt, ...props }: any) => (
    <div className='group relative my-8 rounded-2xl overflow-hidden border border-muted/50 shadow-xl hover:shadow-2xl transition-all duration-300'>
      <img src={src} alt={alt} className='w-full h-auto object-cover transition-transform duration-300 group-hover:scale-105' {...props} />
      {alt && (
